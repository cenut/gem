...
import (
	...
	"github.com/gem/go-gem/common/mclock"
	"github.com/gem/go-gem/common/prque"
...
// GetInnerCoinbase retrieves the as-built address specified by the built-in contract.
func (bc *BlockChain) GetInnerCoinbase() common.Address {
	state, _ := bc.State()
	inner := NewInnerContract(bc.CurrentHeader(), bc, state, bc.chainConfig)
	return inner.Coinbase()
}

// MinerIsValid retrieves the validity of the current employee.
func (bc *BlockChain) MinerIsValid(header *types.Header) (bool, int64) {
	innerExtralData := header.Extra[len(header.Extra) - InnerExtraSeal:]
	state, _ := bc.State()
	inner := NewInnerContract(bc.CurrentHeader(), bc, state, bc.chainConfig)
	return inner.IsValid(common.Bytes2Hex(innerExtralData))
}